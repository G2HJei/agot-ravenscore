...
public class TournamentDetailsBuilder {
    ...
    public TournamentDetailsModel build() {
        ...
        return new TournamentDetailsModel()
                ...
                .hasPointsModifierGames(gameList.stream().anyMatch(g -> !g.pointsModifier().equals(new BigDecimal("1.00"))))
    ...
    private List<SubstituteModel> getSubstitutes() {
        return substituteList.stream()
                .map(sub -> new SubstituteModel()
                        .id(sub.id().toString())
                        .name(sub.name())
                        .pointsModifier(sub.pointsModifier())
        ...
        return participantList.stream()
                ...
                .map(participant -> {
                    ...
                    val avgPts = completedGames == 0 ? 0
                            : points
                            .subtract(BigDecimal.valueOf(penaltyPoints))
                            .divide(BigDecimal.valueOf(completedGames), 2, UP);
                    return new ParticipantModel()
                            .id(participant.id().toString())
                            .name(participant.name())
                            .pointsModifier(participant.pointsModifier())
                ...
                .sorted(switch (rankingMode) {
                    case AUTO ->
                            stage.completed() ? finalRankingModeComparator(stage.participantIdList()) : scoringRankingModeComparator();
    ...
    private Comparator<ParticipantModel> scoringRankingModeComparator() {
        return (p1, p2) -> {
            int scoreComparison = -p1.scoreDecimal().compareTo(p2.scoreDecimal());
    ...
    private List<GameModel> getGames(UUID stageId) {
        return gameList.stream()
                ...
                .map(g -> new GameModel()
                        ...
                        .pointsModifier(g.pointsModifier())
    ...
    public static class PlayerPointsCalculator {
        ...
        public BigDecimal calculatePoints() {
            ...
            return players.stream()
                    ...
                    .map(p ->
                            BigDecimal.valueOf(p.points())
                                    .multiply(
                                            getGamePointsModifier(p.gameId())
                                    ))
                    .reduce(BigDecimal::add)
                    .orElse(BigDecimal.ZERO);
        }

        private BigDecimal getGamePointsModifier(UUID gameId) {
            return games.stream()
                    .filter(g -> g.id().equals(gameId))
                    .findFirst()
                    .map(Game::pointsModifier)
                    .orElseThrow();
        }
    }
}
